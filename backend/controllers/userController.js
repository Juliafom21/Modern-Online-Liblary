...
export const register = catchAsyncErrors(async (req, res, next) => {
  ...
  sendToken(user, 200, "Вас успішно зареєстровано!", res);
});

export const login = catchAsyncErrors(async (req, res, next) => {
  const { email, password, role } = req.body;
  if (!email || !password || !role) {
    return next(new ErrorHandler("Заповніть всю форму!", 400));
  }
  const user = await User.findOne({ email }).select("+password");
  if (!user) {
    return next(new ErrorHandler("Неправильно введена пошта або пароль!", 400));
  }
  ...
  sendToken(user, 200, "Успішний вхід!", res);
});
